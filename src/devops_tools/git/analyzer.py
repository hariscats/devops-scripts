...
class GitAnalyzer:
    ...
    def get_repo_info(self):
        ...
        last_commit = self.run_git_command(
            "git log -1 --pretty=format: '%h - %s (%an, %ar)'", self.repo_path
        )
        ...
        first_commit_date = self.run_git_command(
            "git log --reverse --pretty=format: '%ad' --date=short | head -1", self.repo_path
        )
    ...
    def get_contributor_stats(self):
        ...
        for contributor in contributors:
            ...
            # Get first commit date
            cmd = (
                f"git log --author='{name}' --reverse --pretty=format: '%ad' --date=short | head -1"
            )
    ...
    def get_code_churn_by_period(self, period="month", count=12):
        ...
        if period == "month":
            ...
            cmd = "git log --pretty=format: '%ad' --date=format: '%Y-%m' --shortstat"
            churn_data = self.run_git_command(cmd, self.repo_path)
        elif period == "week":
            # Get code churn by week
            cmd = "git log --pretty=format: '%ad' --date=format: '%Y-%U' --shortstat"
            churn_data = self.run_git_command(cmd, self.repo_path)
        else:
            # Default to daily for any other value
            cmd = "git log --pretty=format: '%ad' --date=short --shortstat"
            churn_data = self.run_git_command(cmd, self.repo_path)
        ...
        churn_by_period = defaultdict(
            lambda: {"files_changed": 0, "insertions": 0, "deletions": 0, "commits": 0}
        )
        current_period = None
        ...
        for period, stats in churn_by_period.items():
            result.append(
                {"period": period, **stats, "net_lines": stats["insertions"] - stats["deletions"]}
            )
...
def main():
    ...
    parser.add_argument(
        "--path", "-p", help="Path to the git repository (default: current directory)"
    )
    ...
    commits_parser.add_argument(
        "--branch", "-b", help="Branch to analyze (default: current branch)"
    )
    ...
    files_parser.add_argument(
        "--days", "-d", type=int, default=90, help="Number of days to analyze (default: 90)"
    )
    ...
    churn_parser.add_argument(
        "--count", "-c", type=int, default=12, help="Number of periods to show (default: 12)"
    )
