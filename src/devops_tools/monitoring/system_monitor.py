...
def get_cpu_info():
    ...
    try:
        ...
        if system == "Linux":
            ...
            with os.popen('top -bn1 | grep "Cpu(s)"') as f:  # nosec
                cpu_usage = f.read().strip()
                cpu_info["CPU Usage"] = cpu_usage

        elif system == "Darwin":  # macOS
            # Get number of cores
            with os.popen("sysctl -n hw.ncpu") as f:  # nosec
                cpu_info["CPU Cores"] = f.read().strip()

            # Use top to get CPU load
            with os.popen('top -l 1 | grep "CPU usage"') as f:  # nosec
                cpu_usage = f.read().strip()
                cpu_info["CPU Usage"] = cpu_usage

        elif system == "Windows":
            # Using WMIC on Windows
            with os.popen("wmic cpu get NumberOfCores") as f:  # nosec
                output = f.read().strip().split("\n")
                if len(output) > 1:
                    cpu_info["CPU Cores"] = output[1].strip()

            # Get CPU load percentage
            with os.popen("wmic cpu get LoadPercentage") as f:  # nosec
                output = f.read().strip().split("\n")
                if len(output) > 1:
...
def get_memory_info():
    ...
    try:
        ...
        if system == "Linux":
            ...
            with os.popen("vm_stat") as f:  # nosec
            ...
            with os.popen("sysctl -n hw.memsize") as f:  # nosec
            ...
            # Using WMIC on Windows
            with os.popen("wmic OS get TotalVisibleMemorySize /Value") as f:  # nosec
                total = f.read().strip().split("=")[1] if "=" in f.read().strip() else "0"

            with os.popen("wmic OS get FreePhysicalMemory /Value") as f:  # nosec
...
def get_disk_info():
    ...
    try:
        ...
        if system == "Linux" or system == "Darwin":  # Linux or macOS
            ...
            # Using WMIC on Windows
            with os.popen("wmic logicaldisk get DeviceID,Size,FreeSpace") as f:  # nosec
...
def get_network_info():
    ...
    try:
        ...
        if system == "Linux":
            ...
            with os.popen("netstat -ib") as f:  # nosec
                lines = f.readlines()
            ...
            with os.popen("netstat -e") as f:  # nosec
...
def get_process_info():
    ...
    try:
        ...
        if system == "Linux":
            ...
            with os.popen("ps -eo pid,ppid,cmd,%mem,%cpu --sort=-%cpu | head -n 11") as f:  # nosec
                processes = f.read().strip()

        elif system == "Darwin":  # macOS
            # Using ps command on macOS
            with os.popen("ps -eo pid,ppid,command,%mem,%cpu -r | head -n 11") as f:  # nosec
                processes = f.read().strip()

        elif system == "Windows":
            # Using WMIC on Windows
            with os.popen(  # nosec
                "wmic process get Caption,ProcessId,ParentProcessId,WorkingSetSize /Format: csv | sort"
            ) as f:
                processes = f.read().strip()
...
def display_system_monitor():
    ...
    os.system("cls" if platform.system() == "Windows" else "clear")  # nosec
