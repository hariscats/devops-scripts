...
class DockerUtils:
    ...
    def find_dangling_volumes(self):
        ...
        command = "docker volume ls -f 'dangling=true' --format '{{.Name}}|{{.Driver}}|{{.Mountpoint}}'"
        output = self.run_command(command)
    ...
    def monitor_container(self, container_name, duration_seconds=60, interval_seconds=5):
        ...
        print(f"Monitoring container '{container_name}' for {duration_seconds} seconds (interval: {interval_seconds}s)...")
...
def main():
    ...
    list_parser.add_argument('-a', '--all', action='store_true', help='Show all containers (default: running only)')
    ...
    dangling_parser.add_argument('-v', '--volumes', action='store_true', help='Find dangling volumes')
    dangling_parser.add_argument('-r', '--remove', action='store_true', help='Remove found dangling resources')
    ...
    monitor_parser.add_argument('-d', '--duration', type=int, default=60, help='Monitoring duration in seconds (default: 60)')
    monitor_parser.add_argument('-i', '--interval', type=int, default=5, help='Update interval in seconds (default: 5)')
