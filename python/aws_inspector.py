...
class AWSInspector:
    ...
    def get_s3_inventory(self):
        ...
        try:
            ...
            for bucket in response['Buckets']:
                ...
                try:
                    s3_region_client = self._get_client('s3', region if region != 'Unknown' else self._get_default_region())
                ...
                try:
                    ...
                    bucket_data['EncryptionType'] = encryption.get('ServerSideEncryptionConfiguration', {}).get('Rules', [{}])[0].get('ApplyServerSideEncryptionByDefault', {}).get('SSEAlgorithm', 'Unknown')
                except ClientError:
                ...
                try:
                    ...
                    bucket_data['BlockPublicAcls'] = public_access.get('PublicAccessBlockConfiguration', {}).get('BlockPublicAcls', False)
                    bucket_data['BlockPublicPolicy'] = public_access.get('PublicAccessBlockConfiguration', {}).get('BlockPublicPolicy', False)
                    bucket_data['IgnorePublicAcls'] = public_access.get('PublicAccessBlockConfiguration', {}).get('IgnorePublicAcls', False)
                    bucket_data['RestrictPublicBuckets'] = public_access.get('PublicAccessBlockConfiguration', {}).get('RestrictPublicBuckets', False)
                except ClientError:
        ...
        try:
            ...
            for func in response.get('Functions', []):
                ...
                try:
                    ...
                    function_data['ReservedConcurrency'] = concurrency.get('ReservedConcurrentExecutions', 'Unreserved')
                except ClientError:
            ...
            for user in users_response.get('Users', []):
                ...
                for key in access_keys.get('AccessKeyMetadata', []):
                    ...
                    try:
                        ...
                        last_used = key_last_used.get('AccessKeyLastUsed', {}).get('LastUsedDate', 'Never')
                    except ClientError:
                        last_used = 'Error'
                ...
                user_data = {
                    ...
                    'PasswordLastUsed': self._format_timestamp(user.get('PasswordLastUsed', 'Never')),
        ...
        try:
            ...
            for volume in response.get('Volumes', []):
                ...
                volume_data = {
                    ...
                    'AttachTime': self._format_timestamp(attachments[0].get('AttachTime')) if is_attached else 'N/A',
    ...
    def print_inventory_summary(self, inventory):
        ...
        print(f"Total Instances: {total_instances} (Running: {total_running}, Stopped: {total_stopped})")
...
def main():
    ...
    parser.add_argument('--regions', nargs='+', help='AWS regions to inspect (default: all regions)')
    parser.add_argument('--profile', help='AWS profile to use')
    parser.add_argument('--output', help='Output file for full inventory (JSON format)')
    parser.add_argument('--service', choices=['ec2', 'ebs', 's3', 'rds', 'lambda', 'iam', 'all'],
                      default='all', help='Service to inspect (default: all)')
    ...
    if args.service != 'all':
        ...
        if args.service == 'ec2':
            for region in inspector.regions:
                ...
                print(f"Total: {ec2_data['TotalCount']} (Running: {ec2_data['RunningCount']}, Stopped: {ec2_data['StoppedCount']})")
                ...
                for i, instance in enumerate(ec2_data['Instances']):
                    ...
                    print(f"   Launch Time: {instance['LaunchTime']}, Days Running: {instance['DaysRunning']}")
                    print(f"   Public IP: {instance['PublicIP']}, Private IP: {instance['PrivateIP']}")
            ...
            for region in inspector.regions:
                ...
                print(f"Unused: {ebs_data['UnusedCount']}, Unencrypted: {ebs_data['UnencryptedCount']}")
                
                # Print volume details
                for i, volume in enumerate(ebs_data['Volumes']):
                    print(f"\n{i+1}. {volume['VolumeId']}")
                    print(f"   Type: {volume['VolumeType']}, Size: {volume['Size']} GB, State: {volume['State']}")
            ...
            for i, bucket in enumerate(s3_data['Buckets']):
                ...
                print(f"   Encrypted: {bucket['Encrypted']}, Encryption Type: {bucket['EncryptionType']}")
                print(f"   Public Access Blocked: {bucket['BlockPublicAcls']} (ACLs), {bucket['BlockPublicPolicy']} (Policy)")
            ...
            for region in inspector.regions:
                ...
                for i, db in enumerate(rds_data['DBInstances']):
                    ...
                    print(f"   Engine: {db['Engine']} {db['EngineVersion']}, Class: {db['DBInstanceClass']}")
                    print(f"   Storage: {db['AllocatedStorage']} GB, Type: {db['StorageType']}")
                    print(f"   Multi-AZ: {db['MultiAZ']}, Encrypted: {db['Encrypted']}")
                    print(f"   Public Access: {db['PubliclyAccessible']}, Backup Retention: {db['BackupRetention']} days")
            ...
            for region in inspector.regions:
                ...
                for i, func in enumerate(lambda_data['Functions']):
                    ...
                    print(f"   Code Size: {func['CodeSize']}, Last Modified: {func['LastModified']}")
                    print(f"   Environment Variables: {func['Environment']}, Reserved Concurrency: {func['ReservedConcurrency']}")
                
        elif args.service == 'iam':
            iam_data = inspector.get_iam_inventory()
            print(f"\nIAM Resources:")
            print(f"Users: {iam_data['UserCount']}, Roles: {iam_data['RoleCount']}, Policies: {iam_data['PolicyCount']}")
